...
export default function SuggestionList({ userId, userLevel }: SuggestionListProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = userLevel >= 1
                  ? await axios.get(`${ipurl}/bloqueios`, config)
                  : await axios.get(`${ipurl}/user/sugestoes/${userId}`, config);
