          ...
          <tbody>
            {historico.map((bloq) => (
              <tr
                ...
                >
                <td>{bloq?.acao != 'update_bloqueio_desbloqueado'? <LockClosedIcon className="w-6 h-6 text-red-500"/> : <LockOpenIcon className="w-6 h-6 text-green-500"/>}</td>
                <td>{bloq?.acao == 'update_bloqueio_desbloqueado'? 'Desbloqueio' : 'Bloqueio'}</td>
                <td>{formatarData(bloq?.dataHora)}</td>
                <td>{bloq?.autorNome  }</td>
                <td>{bloq?.justificativa != null ? bloq?.justificativa : 'Log gerado pelo Sistema'}</td>
                <td>{bloq?.alvo == ""? 'Evento do Sistema':bloq?.alvo }</td>
