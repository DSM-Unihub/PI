...
import atualizarArquivoBloqueadosTotal from "../utils/arquivoBloqueadosTotal.js";
...
async createBlock(data, idUser) {
  try {
    ...
    const bloqueioData = {
      ...
      bloqueioTotal: data.bloqueioTotal === true,
    };
    ...
    // Listagem mitm (bloqueados_total.txt) só via POST /bloqueios/total — não há mais sync automático em /bloqueios.
...
/**
 * Igual createBlock no MongoDB, e sincroniza bloqueados_total.txt (bloqueio total no mitm).
 */
async createManualTotalBlock(data, idUser) {
  const salvo = await this.createBlock(data, idUser);
  await atualizarArquivoBloqueadosTotal(salvo.urlWeb, Boolean(salvo.flag));
  return salvo;
}
...
/**
 * Atualização de registro + sync com bloqueados_total.txt (estado atual da flag).
 */
async updateManualTotalBlock(id, data, idUser) {
  const atualizado = await this.updateBlock(id, data, idUser);
  await atualizarArquivoBloqueadosTotal(atualizado.urlWeb, Boolean(atualizado.flag));
  return atualizado;
}
...
async deleteManualTotalBlock(id, idUser) {
  const bloqueioExcluido = await Indexacao.findById(id);
  if (!bloqueioExcluido) {
    throw new Error("Bloqueio não encontrado");
  }
  await atualizarArquivoBloqueadosTotal(bloqueioExcluido.urlWeb, false);
  return this.deleteBlock(id, idUser);
}
...
async lookupIndexacao(url) {
  try {
    if (!url) {
      throw new Error("URL é obrigatória");
    }

    const normalized = this.normalizeLookupUrl(url);
    if (!normalized) return null;

    const escaped = normalized.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

    const indexacao = await Indexacao.findOne({
      $or: [
        { urlWeb: normalized },
        { urlWeb: `http://${normalized}` },
        { urlWeb: `https://${normalized}` },
        { urlWeb: { $regex: `(^|//|\\.)${escaped}(/|$)`, $options: "i" } },
      ],
    }).sort({ dataHora: -1 });

    return indexacao;
  } catch (error) {
    console.error("Erro ao buscar indexação por lookup de URL:", error);
    throw new Error(error.message || "Erro ao buscar indexação por lookup de URL");
  }
}
