...
const Lockdown = () => {
  ...
  const [doisMesesAtras, setDoisMesesAtras] = useState([]);
  const [variacao, setVariacao] = useState("");
  const fetchBloqueios = async () => {
    try {
      const response = await axios.get(`${url}/estatisticas-mes`);
      setBloqueios(response.data.mesAtual);
      setMesAnterior(response.data.mesAnterior);
      setDoisMesesAtras(response.data.doisMesesAtras);
      setVariacao(response.data.porcentagemVariacao);
    } catch (error) {
  ...
  return (
    <section className="component-container">
        ...
        <div className="grid grid-cols-3">
          <div className="bg-white flex flex-row rounded-bl-xl p-3 xl:p-5 gap-2">
            ...
            <p className="text-sm self-center text-azul-title xl:text-nowrap lg:text-wrap">
              <span className=" text-azul-title text-3xl">462</span> bloq.
              totais
              <br />
          ...
          <div className="bg-azul-gradiente-final flex flex-col rounded-br-xl items-center justify-center">
            <p className="text-base text-white">
              <span className="text-4xl lg:text-3xl">381</span> em Setembro
            </p>
            <p className="text-base text-white">
              <span className="text-4xl lg:text-3xl">428</span> em Outubro
            </p>
          </div>
        </div>
      </div>
      <h3 className="title lg:hidden">Bloqueios totais</h3>
        ...
        <p className="text-3xl font-bold text-azul-cinza-escuro">{bloqueios.bloqueios}</p>
        <p className="text">
          Bloqueios totais <br />
          <span className="text-laranja-s">+{variacao}%</span> este mês
        </p>
