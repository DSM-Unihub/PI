...
class TratamentoDados:
    ...
    def remove_html_tags(html):
        """
        Remove todas as tags HTML do conteúdo HTML.
        :param html: O conteúdo HTML.
        :return: O conteúdo sem as tags HTML.
        """
        soup = BeautifulSoup(html, 'html.parser')
        return soup.get_text()

    def store_bloqueado_notification(self, url):
        # Envia notificação de bloqueio para o cliente via SSE
        try:
            response = requests.post("http://127.0.0.1:5000/notify_blocked", json={"url": url})
            response.raise_for_status()
        except requests.RequestException as e:
